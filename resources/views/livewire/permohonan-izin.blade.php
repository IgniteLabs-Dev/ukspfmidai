                     ...
                     <td class="px-6 py-4 text-sm text-gray-900">
                         <div class="flex justify-center gap-1">
                             @if ($item->status === 'waiting')
                                 ...
                                 <x-button wire:anow-confirm.reject="Apakah anda yakin ingin menolak?" wire:click="reject({{ $item->izin->id }})" bg="[var(--danger)]" px="1"
                                           py="1" label='<i class="fa-solid fa-circle-xmark"></i>' />
                                 <x-button wire:anow-confirm="Apakah anda yakin?" wire:click="approve({{ $item->izin->id }})" bg="[var(--success)]"
                                           px="1" py="1" label='<i class="fa-solid fa-circle-check"></i>' />
                             @else
                                 <x-button wire:anow-confirm="Apakah anda yakin?" wire:click="backToWaiting({{ $item->izin->id }})" bg="[var(--warning)]"
