<div>
    ...
    <div class="relative flex items-center space-x-4">
        <div class="items-center flex flex-wrap">
            <ol
                class="items-start grid grid-flow-col auto-cols-fr overflow-hidden rounded-lg text-sm text-gray-600 me-3
                            ...
                            <div class="flex-row flex items-center gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    <x-button wire:click="resetInput({{ $item->id }})" bg="[var(--danger)]"
                                        px="1" py="0.5" label='<i class="fa-solid fa-sm fa-x"></i>' />
                                    <x-button wire:click="update({{ $item->id }})" bg="[var(--success)]"
                                        px="1" py="0.5" label='<i class="fa-solid fa-sm fa-check"></i>' />
                                </div>
                            </div>
                        @endif
                ...
                @if ($mode == 'create')
                    <div class="px-3 w-auto items-center justify-between">
                        <div class="flex-row flex items-center gap-2 border border-2 border-dashed bg-gray-100 border-gray-300 rounded-md p-2">
                            <div class="">
                                <x-select  placeholder="Pilih Jabatan" for="jabatan_id" wire="jabatan_id" :options="$jabatanTypes"
                                    :required="true" />
                            </div>
